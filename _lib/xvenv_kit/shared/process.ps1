...
function ConvertTo-XvenvWindowsArgument {
    param(
...
function ConvertTo-XvenvWindowsArguments {
    param(
        [Parameter(Mandatory = $true)]
    ...
    $Encoded = foreach ($Argument in $Arguments) {
        ConvertTo-XvenvWindowsArgument ([string]$Argument)
    }
    return [string]::Join(' ', [string[]]@($Encoded))
}

function Invoke-XvenvConsoleProcess {
    param(
    ...
    $StartInfo.Arguments = ConvertTo-XvenvWindowsArguments $Arguments
    ...
    # The child inherits this console, including stdin, stdout, and stderr.
