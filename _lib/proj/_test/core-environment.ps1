...
function Publish-ProjCoreEnvironmentFixture {
    ...
    $Cmd = @(
        ...
        "set `"SWAWKIT_DEV_GENERATION_ID=$Placeholder`""
        "set `"SWAWKIT_DEV_ENV_ROOT=$EnvironmentRoot`""
    ) -join "`r`n"
    $Cmd += "`r`n"
    $Ps1 = @(
        '# Generated by Swaw Kit Proj. Re-run ".dev.setup"; do not edit.'
        "`$env:SWAWKIT_DEV_GENERATION_ID = '$Placeholder'"
        ...
        "`$env:SWAWKIT_TEST_CORE_ENV_MARKER = '$Marker'"
        (
...
'@
    ...
    [IO.File]::WriteAllText(
        ...
        @'
$ErrorActionPreference = 'Stop'
$KernelRoot = Join-Path $env:SWAWKIT_PROJ_HOME '_lib\proj'
. (Join-Path $KernelRoot '_core\engine.ps1')
$Context = Get-ProjProjectContext -ProjHome $env:SWAWKIT_PROJ_HOME
...
'@
    ...
    [IO.File]::AppendAllText(
        ...
        "`$env:SWAWKIT_TEST_CORE_ENV_MARKER = 'tampered'`r`n"
    )
    ...
    $env:SWAWKIT_DEV_ENV_SCHEMA = 'swawkit.proj-dev.environment.v0'
    $env:SWAWKIT_DEV_PROJECT_ROOT = $ProjectRoot
    $env:SWAWKIT_DEV_ENV_ROOT = $EnvironmentRoot
    ...
    $env:SWAWKIT_DEV_GENERATION_ID = $NewGenerationId
