...
try {
    ...
    $AlphaEnvironmentRoot = Join-Path $AlphaRoot 'dev_env'
    [void][IO.Directory]::CreateDirectory($AlphaEnvironmentRoot)
    [IO.File]::WriteAllText(
        (Join-Path $AlphaEnvironmentRoot 'env.cmd'),
        "rem Generated by Swaw Kit Proj.`r`nset X=$AlphaRoot`r`n"
    )
    [IO.File]::WriteAllText(
        (Join-Path $AlphaEnvironmentRoot 'env.ps1'),
        "# Generated by Swaw Kit Proj.`r`n`$env:X = '$AlphaRoot'`r`n"
    )
    ...
    Assert-ProjDataRootTest `
        -Condition ($Claims.Rename -and $Claims.Context -and
            ...
            -not [IO.Directory]::Exists($AlphaRoot) -and
            -not [IO.File]::Exists((Join-Path $BetaRoot 'dev_env\env.cmd')) -and
            -not [IO.File]::Exists((Join-Path $BetaRoot 'dev_env\env.ps1'))) `
        -Message 'rename claim omitted context or retained stale generated paths'
