...
wsl01.cmd ctl dir config
wsl01.cmd ctl dir install
wsl01.cmd ctl install --dry-run
wsl01.cmd ctl install D:\x.tar --dry-run
wsl01.cmd ctl backup D:\x.tar
wsl01.cmd ctl backup list
wsl01.cmd ctl alive status
wsl01.cmd ctl alive 600
wsl01.cmd ctl alive
wsl01.cmd ctl alive off
...
wsl01.cmd vm
wsl01.cmd vm status
wsl01.cmd vm default
wsl01.cmd vm show
wsl01.cmd vm -s
wsl01.cmd vm alive list
wsl01.cmd vm alive off alive_wsl01
wsl01.cmd code ~
```
...
`ctl backup` 的导出格式由入口文件中的 `WSL_export_format` 固定控制，默认是 `tar`；不传路径时导出到备份目录，传路径时导出到指定文件。
`ctl install <archive>` 会用归档创建/重建当前入口绑定的实例名和安装目录，归档可以写绝对路径，也可以只写当前备份目录中的文件名；实例已存在时需要追加 `--yes`，可先用 `--dry-run` 预览原生 `wsl.exe` 命令。
`ctl backup list` 会列出当前实例备份目录中的 `.tar` / `.tar.gz` / `.tar.xz` / `.tgz` / `.vhd` / `.vhdx` 归档。
`ctl alive` 使用当前用户任务计划创建唯一的 alive 槽位：`ctl alive <seconds>` 要求秒数不小于 10，任务运行结束后会自动删除；`ctl alive` 则立即保活并在当前 Windows 用户登录后自动恢复保活。两种设置互相覆盖；`ctl alive off` 会停止并删除该任务计划槽位；`ctl alive status` 会显示实例运行态、当前 alive 设置和任务状态。`ctl remove --yes` 会先删除当前实例的 alive 任务；若入口文件已删除，可用任意入口的 `vm alive list` 查看所有 alive 任务，再用 `vm alive off <list 中显示的任务名>` 删除。
`ctl dir install` 可打开当前实例安装目录；`ctl dir backup` 可打开当前实例备份目录；`ctl dir downloads` 可打开 fallback 镜像缓存目录；`ctl dir ssh` 可打开实例级 `/etc/ssh` 配置目录；`status` 会显示当前实例备份、备份根目录和下载缓存占用。
在线发行版安装默认仍优先使用原生 `wsl --install`；如果失败，会自动尝试 fallback，fallback 会使用 `_lib\wsl_instance_kit\DistributionInfo.json`。
...
`ctl systemd enable` / `ctl systemd disable` 负责写入实例内 `/etc/wsl.conf`；修改后通常需要 `vm -s` 让 WSL 重启后生效。
`vm status` 会显示当前 Windows 用户下的 WSL VM 概览、实例列表、网络模式和 WSL Settings 可用性；`vm default` 会把此入口绑定的实例设置为当前 Windows 用户默认 WSL 实例。
`vm` 会打开 WSL Settings 可视化配置程序，用于处理 WSL VM 网络等当前 Windows 用户级配置；`vm show` 会打开 WSL 欢迎/功能介绍页面；`vm -s` 会关闭当前用户的 WSL 底层 VM/全部实例。
...
测试脚本分两层：`_lib\wsl_instance_kit\test\smoke.ps1` 默认不改环境；`_lib\wsl_instance_kit\test\live.ps1 -Yes` 会真实安装、shutdown、SSH 连接、backup、通过 `ctl install <archive>` 从导出包恢复并删除 `wslkit-live-*` 测试实例。
